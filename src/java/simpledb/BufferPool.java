...
public class BufferPool {
	...
	private Map<PageId, Page> pagesPool;
	...
	public BufferPool(int numPages) {
		...
		this.pagesPool = new HashMap<PageId, Page>(numPages);
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		if(pagesPool.containsKey(pid)) {
			cachedPage= pagesPool.get(pid);
			pageList.remove(cachedPage);
		}
		else {
			...
			pagesPool.put(cachedPage.getId(), cachedPage);
		}
		pageList.addFirst(cachedPage);
		return cachedPage;
...
public void insertTuple(TransactionId tid, int tableId, Tuple t)
	...
	for (Page page : affectedPages) {
		...
		if (pagesPool.get(pageId) == null) {
			pagesPool.put(pageId, page);
			numPages++;
		}
	}
...
public synchronized void discardPage(PageId pid) {
	/*
	 * First we need to locate the page in the Page pool and make sure it is not
	 * dirtied
	 */
	Page pageToDelete = pagesPool.get(pid);
	if (pageToDelete.isDirty() == null) {
		pagesPool.remove(pid);
		pageList.remove(pageToDelete);
	}
}
...
private synchronized void flushPage(PageId pid) throws IOException {
	Page pageToFlush = pagesPool.get(pid);
	int tableId = pid.getTableId();
	DbFile dbFile = Database.getCatalog().getDatabaseFile(tableId);
	dbFile.writePage(pageToFlush);
	pageToFlush.markDirty(true,null);
}
...
private synchronized  void evictPage() throws DbException {
	Page lastPage=pageList.getLast();
	if(lastPage.isDirty()!=null) {
		try {
			flushPage(lastPage.getId());
		}
		catch(IOException IOE) {
			System.err.println("Cannot flush the page"+lastPage+"into the disk");
			IOE.printStackTrace();
		}
	}
	pageList.removeLast();
	pagesPool.remove(lastPage.getId());
}
